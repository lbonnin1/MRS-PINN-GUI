...
class traitement:
    def __call__(self, spectre_corrige_methode_5):
        # Votre traitement ici
        # Notez que je déplace le code de votre méthode `traitement` ici
        ...
        valeurs_numeriques_PPM=[]
        with open(basisset, 'r') as file:
            reader = csv.reader(file,delimiter=';')
            next(reader)
            for row in reader:
                valeurs_numeriques_PPM.append(float(row[0])) 
        valeurs_numeriques_PPM=np.array(valeurs_numeriques_PPM)
        ...
        spectre_corrige_methode_5_shifted_ = np.abs(spectre_corrige_methode_5_shifted)

        return spectre_corrige_methode_5_shifted_